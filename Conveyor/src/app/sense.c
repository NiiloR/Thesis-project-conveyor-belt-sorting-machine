...
void sense_init(void) {
    ...
    // Start continuous ranging; interrupts signal events. Use s_last_meas_ms to cadence APDS sampling
...
static void compute_length(uint32_t t_enter, uint32_t t_exit, LengthInfo* out) {
    ...
    uint32_t mm = (dwell * (uint32_t)belt) / 1000U;
...
static inline void end_session(uint32_t end_ms) {
    ...
    // Turn off presence LED when session ends
...
static inline void finalize_result(SenseResult* out) {
    ...
    if (s_color_sample_count > 0) {
        raw_r = (uint16_t)(s_col_r_sum / s_color_sample_count);
        raw_g = (uint16_t)(s_col_g_sum / s_color_sample_count);
        raw_b = (uint16_t)(s_col_b_sum / s_color_sample_count);
        raw_clear = (uint16_t)(s_col_c_sum / s_color_sample_count);
    } else {
        apds9960_read_rgbc(&raw_r, &raw_g, &raw_b, &raw_clear);
    ...
    Color col = apds9960_classify(raw_r, raw_g, raw_b, raw_clear);
    out->color = col;
    out->ambiguous = is_ambiguous;
    char cbuf[112];
    snprintf(cbuf, sizeof(cbuf), "color: n=%u r=%u g=%u b=%u c=%u class=%u amb=%u\r\n", (unsigned)s_color_sample_count, raw_r, raw_g, raw_b, raw_clear, (unsigned)col, (unsigned)is_ambiguous);
    uart_write(cbuf);
}
...
int sense_poll(SenseResult* out) {
    ...
    bool have_event = vl6180_event();
    // Fallback: if GPIO1/INT is already LOW, we may have missed the falling edge
    // (or the interrupt latched low). In falling-edge mode that can otherwise
    // stall detection permanently.
    if (!have_event && (vl6180_int_pin_level() == 0)) {
        have_event = true;
    }
